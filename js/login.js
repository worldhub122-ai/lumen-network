document.addEventListener('DOMContentLoaded', function(){
  ...
  document.body.addEventListener('click', (e)=>{
    const el = e.target.closest('[data-action="stub"]');
    if(el){ e.preventDefault(); showToast(el.dataset.msg || 'Not wired up yet.'); }
  });

  /* ============================================================
     RATING BADGE (matches post/profile meter styling)
     ============================================================ */
  ...
  function paintCaption(){
    if(!window.LumenI18n) return;
    const dict = window.LumenI18n.translations[currentLang] || window.LumenI18n.translations.en;
    if(captionEl) captionEl.textContent = dict[captionKeys[captionIndex]];
  ...
  if(pwToggle && pwInput){
    pwToggle.addEventListener('click', ()=>{
      const dict = (window.LumenI18n && window.LumenI18n.translations[currentLang]) || {};
      const showing = pwInput.type === 'text';
      pwInput.type = showing ? 'password' : 'text';
      pwToggle.setAttribute('aria-pressed', String(!showing));
      pwToggle.setAttribute('aria-label', showing ? (dict.showPassword || 'Show password') : (dict.hidePassword || 'Hide password'));
      pwToggle.querySelector('.i-eye').hidden = !showing;
      pwToggle.querySelector('.i-eye-off').hidden = showing;
  ...
  if(form){
    form.addEventListener('submit', (e)=>{
      ...
      submitBtn.querySelector('span').textContent = dict.signingIn || 'Signing in…';
      setTimeout(()=>{
        submitBtn.disabled = false;
        submitBtn.querySelector('span').textContent = dict.signIn || 'Sign in';
